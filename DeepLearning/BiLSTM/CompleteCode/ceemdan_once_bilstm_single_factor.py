...
def ceemdan_cnn(day):
    ...
    reshape1 = 0
    IMF = pd.read_excel('../../../Data/CEEMDAN/SingleFactor_20years_ET0/58693/'+'CEEMDAN_58693 Station_'+day+'.xlsx',header=None)
    ...
    for imf_run in range(int(len(IMF.columns))):
        ...
        def BiLSTM(optimizer='adam', batch_size=32, epochs=40):
            model = Sequential()
            model.add(Bidirectional(LSTM(150, activation='relu', input_shape=(1, 1))))
...
def plot_save_true_prediction(reshape1,finally_y_true,finally_pre_rsult,day,n_steps_out,formatted_time):
    ...
    # 保存为CSV文件
    true_file_path = "../Result/CEEMDAN_Single_Factor_20_years/58693_2/"+day+"_day_CEEMDAN_IMF" + str(reshape2 - 1) + "-BiLSTM_true.csv"
    ...
    prediction_file_path = "../Result/CEEMDAN_Single_Factor_20_years/58693_2/"+day+"_day_CEEMDAN_IMF" + str(reshape2 - 1) + "-BiLSTM_prediction.csv"
    # 保存为CSV文件
    ...
    with open('ceemdan_bilstm_58693_2_statistical_indicator.csv',mode='a',newline='') as file:
