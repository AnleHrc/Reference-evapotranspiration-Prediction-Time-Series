...
def CNN_BiLSTM_ET0(filename):
    ...
    def CNN_BiLSTM(optimizer='adam', batch_size=32, epochs=40):
        ...
        model.add(TimeDistributed(
            Conv1D(filters=256, kernel_size=1, activation='relu', input_shape=(None, n_past, sacler_data_len), padding='same')))
    ...
    # mode = 'w' :覆盖.mode= 'a' ： 追加
    with open('cnn_bilstm_58606_3_statistical_indicator.csv',mode='a',newline='') as file:
    ...
    result_save_file_true.to_csv('../Result/ET0_SingleFactor_20years_Result/58606_3/' + day + '_day_true.csv', index_label='Time')
    result_save_file_pred = pd.DataFrame(preds)
    result_save_file_pred = result_save_file_pred.rename(columns={0: "Pred"})
    result_save_file_pred.to_csv('../Result/ET0_SingleFactor_20years_Result/58606_3/' + day + '_day_prediction.csv', index_label='Time')


import time
start_time = time.time()

csv_path = '../../../Data/ProcessedData/SingleFactorData/58606/'
...
mixer.init()
mixer.music.load('../../../Resources/Abstract_Experimental_Track__2__looped_.WAV')
