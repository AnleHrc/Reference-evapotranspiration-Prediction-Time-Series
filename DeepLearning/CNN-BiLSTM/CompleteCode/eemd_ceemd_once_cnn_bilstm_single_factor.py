...
def ceemdan_cnn(day,model,station_id):
    ...
    for imf_run in range(int(len(IMF.columns))):
        ...
        def CNN_BiLSTM(optimizer='adam', batch_size=32, epochs=40):
            ...
            model.add(TimeDistributed(Conv1D(filters=256, kernel_size=1, activation='relu', input_shape=(None, 1,sacler_data_len),padding='same')))
...
def plot_save_true_prediction(reshape1,finally_y_true,finally_pre_rsult,day,n_steps_out,formatted_time,model,station_id):
    ...
    # 保存为CSV文件
    true_file_path = f"../Result/{model}_Single_Factor_20_years/{station_id}/" + day + f"_day_{model}_IMF" + str(
        reshape2 - 1) + "-CNN_BiLSTM_true.csv"
    ...
    prediction_file_path = f"../Result/{model}_Single_Factor_20_years/{station_id}/" + day + f"_day_{model}_IMF" + str(
        reshape2 - 1) + "-CNN_BiLSTM_prediction.csv"
    # 保存为CSV文件
    ...
    with open(f'{model}_cnn_bilstm_{station_id}_statistical_indicator.csv', mode='a', newline='') as file:
        ...
        writer.writerow(
            [str(day), statistical_indicator['R2'], statistical_indicator['RMSE'], statistical_indicator['MAE'],
             statistical_indicator['NSE'],
