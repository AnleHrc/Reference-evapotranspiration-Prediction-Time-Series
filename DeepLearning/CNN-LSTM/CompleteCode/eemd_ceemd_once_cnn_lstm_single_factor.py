...
def ceemdan_cnn(day,model,station_id):
    ...
    for imf_run in range(int(len(IMF.columns))):
        ...
        BiLSTM_Model = KerasRegressor(CNNLSTM, epochs=40, verbose=1, validation_data=(testX, testY))
...
def plot_save_true_prediction(reshape1,finally_y_true,finally_pre_rsult,day,n_steps_out,formatted_time,model,station_id):
    ...
    prediction_file_path = f"../Result/{model}_Single_Factor_20_years/{station_id}/" + day + f"_day_{model}_IMF" + str(
        reshape2 - 1) + "-CNN_LSTM_prediction.csv"
    # 保存为CSV文件
    ...
    with open(f'{model}_cnn_lstm_{station_id}_statistical_indicator.csv',mode='a',newline='') as file:
