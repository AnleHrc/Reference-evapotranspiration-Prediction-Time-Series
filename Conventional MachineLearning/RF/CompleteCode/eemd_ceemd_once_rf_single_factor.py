...
def plot_save_true_prediction(reshape1,finally_y_true,finally_pre_rsult,day,n_steps_out,formatted_time,model,station_id):
    ...
    # 保存为CSV文件
    true_file_path = f"../Result/{model}_Single_Factor_20_years/{station_id}/"+day+f"_day_{model}_IMF" + str(reshape2 - 1) + "-RF_true.csv"
    ...
    prediction_file_path = f"../Result/{model}_Single_Factor_20_years/{station_id}/"+day+f"_day_{model}_IMF" + str(reshape2 - 1) + "-RF_prediction.csv"
    # 保存为CSV文件
    ...
    with open(f'{model}_rf_{station_id}_statistical_indicator.csv',mode='a',newline='') as file:
